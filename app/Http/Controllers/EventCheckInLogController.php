...
class EventCheckInLogController extends MyBaseController
{
    /**
    ...
    {
        $allowed_sorts = ['first_name', 'email', 'ticket_id', 'is_hidden', 'is_normal', 'type', 'checkin_at', 'checkout_at', 'clear_checkin_at'];
		...
		$sign_certs = SignCert::where('organiser_id', '=', $event->organiser_id)
        ...
        if ($searchQuery) {
            $attendees = $event->attendees()
                ...
                ->orderBy(($sort_by == 'is_hidden' || $sort_by == 'is_normal' || $sort_by == 'type' ? 'tickets.' : 'attendees.') . $sort_by, $sort_order)
        ...
        $data = [
			...
			'sign_certs' => $sign_certs
        ];

        return view('ManageEvent.CheckInLog', $data);
    }
	...
	public function showPrintCheckinLog(Request $request, $event_id)
    {
        $allowed_sorts = ['first_name', 'email', 'ticket_id', 'is_hidden', 'is_normal', 'type', 'checkin_at', 'checkout_at', 'clear_checkin_at'];
        ...
        return view('ManageEvent.PrintCheckInLog', $data);
    }
    /**
     * Invite an attendee
     *
     ...
     */
    public function showExportCheckinLog($event_id, $export_as = 'xls')
    {

        Excel::create('checkinlog-as-of-' . date('d-m-Y-g.i.a'), function ($excel) use ($event_id) {
            ...
            $excel->sheet('attendees_sheet_1', function ($sheet) use ($event_id) {
                ...
                $data = DB::table('attendees')
                    ...
                    ->select([
                        ...
                        'attendees.last_name',
						DB::raw("(CASE WHEN tickets.is_normal=2 THEN 'Normal' WHEN tickets.is_normal=3 THEN 'Free' ELSE 'Suggested Donation' END) AS has_arrived"),
                        'tickets.type',
						'attendees.email',
						'attendees.checkin_at',
                        'attendees.checkout_at',                        
                        'attendees.clear_checkin_at'
                    ])->get();
