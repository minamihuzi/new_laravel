...
class OrderMailer
{
    ...
    public function sendOrderTickets(Order $order)
    {
        ...
        if (!file_exists($file_path)) {
            Log::error("NSC Cannot send actual ticket to : " . $order->email . " as ticket file does not exist on disk");
            $file_path = public_path(config('attendize.event_pdf_tickets_path')) . '/test.pdf';
			//return;
        }
