...
@section('page_title')
	...
	{{$event->title}}
    @lang("Ticket.event_tickets")
	@lang("instruction.instruction05")
@stop
    ...
    <script>
        $(function () {
			...
			$(".deleteTicket").click(function(){
				var name = prompt("Please input correct pass code:", "0000");
				if(name=={{$pass_code}}){
					if(confirm("Do you really want to delete?")){
						var ticket_id = $(this).attr('data-id');
						$.ajax({
							type: 'POST',
							url: '{{ route('postDeleteTicket' ,['event_id' => $event->id]) }}',
							dataType: 'json',
							data: {ticket_id: ticket_id},
							success: function (data) {
								showMessage(data.message);
								document.location.href="{{route('showEventTickets', array('event_id' => $event->id))}}";
							},
							error: function (data) {
								showMessage(lang("whoops2"));
							}
					...
					alert("You did input invalid pass code. Please input valid pass code.");
				}
			});
        });
    </script>
                    ...
                    <div class="panel panel-success ticket" data-ticket-id="{{$ticket->id}}">
                        <div style="cursor: pointer;" data-modal-id='ticket-{{ $ticket->id }}'
                            ...
                            <h3 class="panel-title">
                                <p style="margin-bottom:3px;text-align:center;">{{$ticket->type}}</p>
								@if($ticket->is_hidden)
                        ...
                        <div class="panel-footer" style="height: 56px; padding-right:5px;">
                            <div class="sortHandle" title="@lang("basic.drag_to_reorder")" style="left:5px;">
                            ...
                            <ul class="nav nav-section nav-justified">
                                <li>
                                    ...
                                    <i class="ico-pause"></i> @lang("Ticket.pause")
                                        ...
                                        @endif
										<span class="label label-info deleteTicket" style="float:right;" data-id="{{$ticket->id}}">
											<i class="ico-delete"></i> @lang("Ticket.delete")
										</span>
                                    </a>
