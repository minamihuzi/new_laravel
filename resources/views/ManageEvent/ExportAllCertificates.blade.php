<html>
    ...
    <body style="background-color: #FFFFFF;" onload="window.print();">     
		...
		<div class="row" id="cert_content_{{$attendee->id}}">
			<script>
				$(document).ready(function() {
					$("#cert_content_{{$attendee->id}} #org_name").html("{{$attendee->full_name}}");
					...
					$("#cert_content_{{$attendee->id}} #org_hrs").html(ceu_val.toFixed(2)+" {{$event->ceu_unit}}");
					$("#cert_content_{{$attendee->id}} #org_name").html("{{$attendee->full_name}}");
				});		
			</script>
			<table width="100%">
